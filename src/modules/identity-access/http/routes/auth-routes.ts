...
import { AuthController } from "@/modules/identity-access/http/controllers/auth-controller";
import { SessionController } from "@/modules/identity-access/http/controllers/session-controller";
...
import { AuthValidation } from "@/modules/identity-access/http/validations/auth-validation";
import { SessionValidation } from "@/modules/identity-access/http/validations/session-validation";
...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post("/register", {
    config: { rateLimit: { max: 5, timeWindow: "15 minutes", errorResponseBuilder: () => rateLimitMessage } },
    preHandler: [httpValidate(AuthValidation.register())],
  }, controller.register.bind(controller));

  app.post("/login", {
    config: { rateLimit: { max: 10, timeWindow: "15 minutes", errorResponseBuilder: () => rateLimitMessage } },
  ...
  app.post("/refresh-token", {
    config: { rateLimit: { max: 30, timeWindow: "15 minutes", errorResponseBuilder: () => rateLimitMessage } },
